...
MAIN_CLASS=com.streamer.worker.main.WorkerApplication
...
GCOPTS="-Dapp.home=$APP_HOME -Xmx4G -Xms128M -XX:+UseG1GC -XX:+PrintGCDetails"
...
if [ -z $APP_PID ] ; then
  if [ ! -d "$JAVA_HOME" ]; then
    nohup $JRE_HOME/bin/java $GCOPTS -cp $CLASSPATH $MAIN_CLASS $@ > $APP_LOG_DIR/application.out 2>&1 &
  else
    nohup $JAVA_HOME/bin/java $GCOPTS -cp $CLASSPATH $MAIN_CLASS $@ > $APP_LOG_DIR/application.out 2>&1 &
  fi
  echo "Using APP_HOME:$APP_HOME"
  echo "Using APP_LOG_DIR:$APP_LOG_DIR"
  sleep 2s
  APP_PID=`ps -ef | grep -v grep | grep $APP_HOME | sed -n  '1P' | awk '{print $2}'`
  echo "Application start , PID:$APP_PID."
else
  echo "Sorry,Application is running now." 
fi
